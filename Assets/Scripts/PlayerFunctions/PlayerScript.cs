...
public class PlayerScript : MonoBehaviour
{
	...
	private void MouseMove()
	{
		...
		playerRotation.eulerAngles = playerRotation.eulerAngles + Vector3.up * Singleton<InputManager>.Instance.GetLookAxisX() * mouseSensitivity * Time.timeScale * flipaturn;
		transform.rotation = playerRotation;
	}
	private void PlayerMove()
	{
		Vector2 moveAxis = Singleton<InputManager>.Instance.GetMoveAxis();
		Vector3 movement = Vector3.ClampMagnitude(transform.forward * moveAxis.y + transform.right * moveAxis.x, 1f);
		if (stamina > 0f)
		{
		...
		moveDirection = movement * playerSpeed;
