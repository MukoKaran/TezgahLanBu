...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!learningActive)
		{
			...
			if (CF2Input.GetAxis("Mouse ScrollWheel") > 0f && Time.timeScale != 0f)
			{
				DecreaseItemSelection();
			}
			else if (CF2Input.GetAxis("Mouse ScrollWheel") < 0f && Time.timeScale != 0f)
			{
	...
	private IEnumerator HitTextFadeOut()
	{
		...
		float fadeTime = Mathf.Max(0.1f, hitTextFadeTime);
		float elapsed = 0f;
	...
	public string hitTextMessage = "you png/text here"; // [MOD] Text shown on screen (edit from the Inspector).
	...
	public float hitTextHoldTime = 3f; // [MOD] How long the hit text stays fully visible.
	public float hitTextFadeTime = 2.5f; // [MOD] How long the hit text fades out.
